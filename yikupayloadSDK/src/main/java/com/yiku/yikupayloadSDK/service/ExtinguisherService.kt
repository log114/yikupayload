...
import com.yiku.yikupayloadSDK.protocol.EXTINGUISHER_OPERATE
import com.yiku.yikupayloadSDK.protocol.EXTINGUISHER_STATE_SEND
...
class ExtinguisherService {
    ...
    open fun sendData2Payload(data: ByteArray) {
        thread {
            try {
                Log.i(TAG, "灭火罐，sendData:${bytesToHex(data)}")
                //向输出流中写入数据，传向服务端
                if (!getIsConnected()) {
                    return@thread
                }
